...
class LSAPLayeredPlannerMOD:
    ...
    def get_moves(self, atom_sites):
        ...
        prios = map(lambda x: self.get_type(targets[x]), col_inds)
        moves_inds = sorted(zip(dists, prios, row_inds, col_inds), reverse=True)
        moves = filter(lambda mv: mv[0] != mv[1], map(lambda x: (atom_sites[x[2]], targets[x[3]]), moves_inds))
...
def main(n_traps, alpha=1):
    ...
    arr_planner = LSAPLayeredPlanner(n_rows, n_cols, targets, metric)
